...
const createWindow = async () => {
  ...
  protocol.interceptFileProtocol('sprites', (request, callback) => {
    ...
    let [,character, emote, file] = filePath.match(/(.*)\/(.*)\/(.*)/);

    let spritePath = `${process.env.HOME}/lame-dimension-assets/assets/sprites/${emote}/spr_${character}${emote}/${file}.png`;

    log.info("FILE PATH: " + spritePath);
...
ipcMain.handle('loadScript', (event) => {
    let scriptDir = `${process.env.HOME}/script.json`;
    let assetDirectory = `${process.env.HOME}/lame-dimension-assets/assets`;
...
ipcMain.handle('getSpriteData', (event, character, emote) => {
    console.log(character + ":" + emote);
    let directory = `${process.env.HOME}/lame-dimension-assets/assets/sprites/${emote}/spr_${character}${emote}`;
